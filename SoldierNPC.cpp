...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	//Physics Components:
	Vector3 sphereCenter = pMainSN->m_base.getPos() + Vector3(0, 1.0f, 0);
	double sphereRadius = 3.0f;
	PhysicsComponent* pPhysicsComponent = new PhysicsComponent(Shape::Type::SPHERE, pMainSN, sphereRadius, sphereCenter);
	pMainSN->pcSN = pPhysicsComponent;
	...
	for (int iSkin = 0; iSkin < numskins; ++iSkin)
	{
		...
		//std::vector<PhysicsComponent*> a = pInstance->getComponents();
	...
	pSoldierBehaviorSM->m_isTarget = pEvt->m_isTarget;
	// start the soldier

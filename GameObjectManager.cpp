...
void GameObjectManager::do_CREATE_MESH(Events::Event *pEvt)
{
	...
	if (!haveObject)
	{
		...
		m_pContext->getGPUScreen()->AcquireRenderContextOwnership(pRealEvent->m_threadOwnershipMask);
			...
			std::string MeshName(pRealEvent->m_meshFilename);
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				pSN->m_base.setV(pRealEvent->m_v);
				pSN->m_base.setN(pRealEvent->m_n);
				pMeshInstance->m_pos = pSN->m_base.getPos();
				pMeshInstance->m_u = pSN->m_base.getU();
				pMeshInstance->m_v = pSN->m_base.getV();
				pMeshInstance->m_n = pSN->m_base.getN();
				Vector3 meshAABB[8];
				for (int i = 0; i < 8; i++) {
					meshAABB[i] = pMeshInstance->m_hAsset.getObject<Mesh>()->m_AABBCoords[i];
				}

				//Box box(meshAABB);
				if (MeshName.find("cobbleplane") != std::string::npos) {
					PhysicsComponent* pPhysicsComponent = new PhysicsComponent(Shape::Type::PLANE, pSN, meshAABB);
					pSN->pcSN = pPhysicsComponent;
					PhysicsManager* pInstance = PhysicsManager::getInstance();
					pInstance->addPhysicsComponent(pPhysicsComponent);
				}
				else {
					PhysicsComponent* pPhysicsComponent = new PhysicsComponent(Shape::Type::BOX, pSN, meshAABB);
					pSN->pcSN = pPhysicsComponent;
					PhysicsManager* pInstance = PhysicsManager::getInstance();
					pInstance->addPhysicsComponent(pPhysicsComponent);
				}
